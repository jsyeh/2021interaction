...
float wallX=250, wallY=250, wallW=130, wallH=150;
void setup(){
  size(500,400);
}
...
void draw(){
  ...
  if( testYWall(wallX,wallY, wallY+wallH)==true ) bulletVX = -bulletVX;
  else if( testYWall(wallX+wallW, wallY, wallY+wallH)==true) bulletVX = -bulletVX;
  else if( testXWall(wallY, wallX, wallX+wallW)==true) bulletVY = -bulletVY;//Wally, x1,x2
  else if( testXWall(wallY+wallH, wallX, wallX+wallW)==true ) bulletVY = -bulletVY;
