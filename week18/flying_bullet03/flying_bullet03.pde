...
void draw(){
  ...
  for(int i=0; i<N; i++){
    ...
    if( flying && dist(x,y, enemyX[i],enemyY[i])<30 ){
      flying=false;
      if(enemyC[i]==c){
        findSameNeighborAndDeleteOrAddEnemy(i,c);//發現有相同的顏色耶!!!! 要判斷數有幾個鄰居不需要 Add Enemy囉!!!
        break;//以免拿newBullet()的位置又去找格子黏
      }else findIJandAddEnemy();//加敵人之外,可能也要判斷是不是可以消掉!!!
    }//子彈變成敵人的一部分
  }
  fill( table[c] );
  ellipse(x,y, 30,30);
...
void findSameNeighborAndDeleteOrAddEnemy(int enemyI, int c){
  ...
  for(int i=0; i<N; i++){
    if( i!=enemyI && enemyC[i]==c && dist(enemyX[i],enemyY[i], x,y)<=30){
      ...
      //接下來,便把自己消掉!!!
      delete(i, enemyI);
      newBullet();//good!有消掉,不要留下子彈
      return ;
    }
  }
  findIJandAddEnemy();//bad,沒有任何相同的可以消掉, 只好留下子彈當敵人
}
void delete(int a){
  for(int i=a; i<N-1; i++){
    enemyX[i]=enemyX[i+1];
